...
namespace searchEngines
{
    public class loserTree
    {
        ...
        public void sort()                             //外排序,建倒排索引
        {
            ...
            else if(n==1)
            {
                while (true)
                {
                    ...
                    if (CurrentKey.CompareTo(LastKey) != 0)
                    {
                        keywordTable[CurrentKey] = outputFile.Position;
                        LastKey = CurrentKey;
                    }
                    writer.WriteLine(CurrentKey + " " + L[1].fNum + " " + L[1].posInFile);    //写入胜者
                    writer.Flush();
                    if (Positions[1] < Boundaries[2])    //没有冲突
                    {
                        L[1] = getData(1);
                    }
                    else break;
        ...
        {
            if (keywordTable[word] == null) return null;
            long address = (long)keywordTable[word];
            FileStream strm = new FileStream("index3.txt", FileMode.Open, FileAccess.Read);
            strm.Seek(address, SeekOrigin.Begin);
            StreamReader reader = new StreamReader(strm, Encoding.Unicode);
            KEY key = new KEY();
            ...
            while (key != null && key.word == word)
            {
                ...
                for (i = 0; i < list.Count && list[i].file <= fp.file; i++)
                {
                    ...
                    {
                        list[i].poslist = list[i].poslist.Union<long>(fp.poslist).ToList<long>();
                        break;
                    }
                }
