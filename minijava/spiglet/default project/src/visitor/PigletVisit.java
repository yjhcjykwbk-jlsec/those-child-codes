...
public class PigletVisit extends GJDepthFirst<Integer,Object> {
	...
	public Integer visit(AssignmentStatement n, Object s) {
		...
		if (func.declaredInFunc(name))
		{
			...
			if(temp!=TEMPReg)
			Println("MOVE TEMP " + (temp) + " TEMP " + TEMPReg);
		}
		/* declared in class,and stored in memory from TEMP0 4 to 4*n */
			...
			if (_table != table) {
				 ...
				 * IDENTIFIER-VISIT!!
				 */
				Println(("HSTORE TEMP " + classReg+ " " + (_table.getNum(name) * 4 + " TEMP " + resReg)));
			} 
			else {
				Println(("HSTORE TEMP 0 " + (_table.getNum(name) * 4 + " TEMP " + TEMPReg)));
			}
		}
		return null;
	}
	...
	public Integer visit(ArrayAssignmentStatement n, Object s) {
		...
		Println(("CJUMP TEMP "+jumpReg+" NER" + (++noerror)));
		
		Println(("ERROR"));
		Println(("NER" + noerror + " NOOP"));
		...
		reg = stack.pop();
		int indexReg = (++reg);
		if(indexReg==TEMPReg) indexReg=(++reg);
			Println(("MOVE TEMP " + (indexReg) + " PLUS TEMP " + (TEMPReg)+" 1"));
		...
		/* index=4*indexReg */
		TEMPReg=(reg+1);/**/
		Println(("MOVE TEMP " + (TEMPReg) + " TEMP " + (indexReg)));
		Println(("MOVE TEMP " + (indexReg) + " TIMES TEMP " + (TEMPReg)+" 4"));
	...
	public Integer visit(IfStatement n, Object s) {
		...
		//int jumpReg=(++reg);
		//Println(("MOVE TEMP "+jumpReg+" LT TEMP "+ zeroReg +" TEMP " + (ifReg)));
		//Println(("CJUMP TEMP " + (jumpReg) + " label" + (elselabel)));
		/* if 0==ifReg,goto else */
		Println(("CJUMP TEMP " + (ifReg) + " label" + (elselabel)));
		n.f4.accept(this, s);
	...
	public Integer visit(WhileStatement n, Object s) {
		...
		/* if 1!=ifReg ,break */
		Println(("CJUMP TEMP " + (ifReg) + " label" + (outlabel)));
		n.f4.accept(this, s);
	...
	public Integer visit(AndExpression n, Object s) {
		...
		Println(("MOVE TEMP "+jumpReg+" LT TEMP "+TEMPReg+" TEMP " + (resReg)));
		Println(("CJUMP TEMP " + (jumpReg) + " label" + (outlabel)));
...
/**
    ...
    */
	public Integer visit(CompareExpression n, Object s) {
		...
		/*int resReg = (++reg);*/
		Println(("MOVE TEMP " + leftReg + " LT TEMP " + leftReg + " TEMP " + rightReg));
		return leftReg;
	}
	...
	public Integer visit(PlusExpression n, Object s) {
		...
		/**int resReg = (++reg);**/
		Println(("MOVE TEMP " + leftReg + " PLUS TEMP " + leftReg + " TEMP " + rightReg));
		return leftReg;
	}
	...
	public Integer visit(MinusExpression n, Object s) {
		...
		/*int resReg = (++reg);*/
		Println(("MOVE TEMP " + leftReg + " MINUS TEMP " + leftReg + " TEMP " + rightReg));
		return leftReg;
	}
	...
	public Integer visit(TimesExpression n, Object s) {
		...
		//int resReg = (++reg);
		Println(("MOVE TEMP " + leftReg + " TIMES TEMP " + leftReg + " TEMP " + rightReg));
		return leftReg;
	}
	...
	public Integer visit(ArrayLookup n, Object s) {
		...
		int resReg = ++reg;
		TEMPReg=(reg+1);/*must be specialized*/
		Println(("MOVE TEMP " + (TEMPReg) + " TIMES TEMP " + indexReg+ " 4"));
		Println(("MOVE TEMP " + (indexReg) + " TEMP " + TEMPReg));
		Println(("MOVE TEMP " + (TEMPReg) + " PLUS TEMP " + (arrayReg) + " TEMP " + indexReg));
		Println(("HLOAD TEMP " + (arrayReg) + " TEMP " + (TEMPReg) + " 0"));
		return arrayReg;
	}
	...
	public Integer visit(ArrayLength n, Object s) {
		...
		Println(("ERROR"));
		Println(("NER" + noerror + " NOOP"));
		Println(("HLOAD TEMP " + (jumpReg) + " TEMP " + (arrayReg) + " 0"));
		return jumpReg;
	}
	...
	public Integer visit(MessageSend n, Object s) {
		...
		Println(("HLOAD TEMP " + (funcReg) + " TEMP " + (functableReg) + " " + (8 * _table.func.getNum(funcname))));
		/*store scope*/
		Println(("HLOAD TEMP " + (classReg) + " TEMP " + (functableReg) + " " + (8 * _table.func.getNum(funcname)+4)));
		int beginReg = reg;
		n.f4.accept(this, s);
		/* call function */
		Println(("MOVE TEMP " + resReg + " CALL TEMP " + funcReg + " ( TEMP "+ classReg + " "));
	...
	public Integer visit(Identifier n, Object s) {
		...
		if (func.declaredInFunc(name)) {
			...
			if (_table != table) {
				 ...
				 */
				Println(("HLOAD TEMP " + (resReg) + " TEMP " + classReg + " " + (_table.getNum(name) * 4)));
			} 
			else
				Println(("HLOAD TEMP " + (resReg) + " TEMP 0 " + (func.getNum(name) * 4)));
		}
		return resReg;
	}
	...
	public Integer visit(ArrayAllocationExpression n, Object s) {
		...
		Println(("MOVE TEMP " + (sizeReg) + " PLUS TEMP " + sizeReg+" 1"));
		/* sizeReg here is the real number of words stored in memory */

		int arrayReg=(++reg);
		Println(("MOVE TEMP " + arrayReg +" TIMES TEMP "+ sizeReg +" 4"));
		Println(("MOVE TEMP " + resReg + " HALLOCATE TEMP " + arrayReg));
		/* TEMPReg=sizeReg-1,store array-size */
		TEMPReg=(reg+1);
		Println(("MOVE TEMP " + (TEMPReg) + " MINUS TEMP " + sizeReg + " 1"));
		Println(("HSTORE TEMP " + resReg + " 0 TEMP " + TEMPReg));
	...
	public Integer visit(AllocationExpression n, Object s) {
		...
		while (fathers.size()>0) {
			...
			for (i = 0; i < funcNum; i++) {
				...
				Println(("MOVE TEMP " + (TEMPReg) + " " + getProcess(curr, i)));
			...
			for (i = 1; i <= varNum; i++) {
				Println(("MOVE TEMP " + (TEMPReg) + " 0 "));
				Println(("HSTORE TEMP " + (classReg) + " " + (4 * i) + " TEMP "+TEMPReg));
			}
		...
		reg = stack.pop();
		Println("MOVE TEMP " + TEMPReg + " TEMP " + fatherRegs.get(_table));
		return TEMPReg;
	}
	...
	public Integer visit(NotExpression n, Object s) {
		...
		TEMPReg=(reg+1);
		Println(("MOVE TEMP " + (TEMPReg) + " 1 " ));
		Println("MOVE TEMP " + resReg + " MINUS TEMP "+TEMPReg+" TEMP " + preReg);
		return resReg;
	}
