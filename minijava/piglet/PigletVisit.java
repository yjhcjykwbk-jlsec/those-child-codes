...
public class PigletVisit extends GJDepthFirst<Integer,tab>{
	...
	public Integer visit(MethodDeclaration n,tab s) {
	   ...
	   public Integer visit(VarDeclaration n, tab s) {
		  ...
		  Println("mov temp "+func.getNum(name)+" 0",s);
	      return null;
	   }
	...
	public Integer visit(ArrayAssignmentStatement n,tab s) {
		...
		reg=stack.pop();
		int arrayReg=(++reg);
		if(arrayReg!=tempReg) Println(("mov temp " + (arrayReg) +" temp "+tempReg),s);
		/**/
		...
		reg=stack.pop();
		int indexReg=(++reg);
		if(indexReg!=tempReg) Println(("mov temp " + (indexReg)+" temp "+tempReg),s);
		...
		if(resReg!=tempReg) Println(("mov temp " + (resReg) +" temp "+tempReg),s);
	...
	public Integer visit(IfStatement n,tab s) {
		...
		reg=stack.pop();
		int ifReg=(++reg);
		if(ifReg!=tempReg) Println(("mov temp " + (ifReg)+" temp "+tempReg),s);
	...
	public Integer visit(AndExpression n,tab s) {
		...
		Println(("cjump lt 0 temp " + (resReg) +" outlabel" + (outlabel)),s);
		...
		Println(("outlabel" + (outlabel) + " NOOP"),s);
		return resReg;
	}
...
/**
    ...
    */
	public Integer visit(CompareExpression n,tab s) {
		...
		reg=stack.pop();
		int leftReg=(++reg);
		if(leftReg!=tempReg) Println(("mov temp " + (leftReg)+" temp "+tempReg),s);
		
		stack.add(reg);
		tempReg=n.f0.accept(this,s);
		reg=stack.pop();
		int rightReg=(++reg);
		if(rightReg!=tempReg) Println(("mov temp " + (rightReg)+" temp "+tempReg),s);
	...
	public Integer visit(ArrayLookup n,tab s) {
		...
		int resReg =++reg;
		Println(("mov temp " + (resReg)+" plus temp "+(arrayReg)+" times 4 temp "+indexReg),s);
	...
	public Integer visit(MessageSend n,tab s) {
		...
		reg=stack.pop();
		int classReg=(++reg);
		if(classReg!=tempReg) Println(("mov temp " + (classReg)+" temp "+tempReg),s);
	...
	public Integer visit(ExpressionList n,tab s) {
		...
		reg=stack.pop();
		int resReg=(++reg);
		if(resReg!=tempReg) Println("return temp "+resReg,s.add());
		Println("end\n",s);
		n.f1.accept(this,s);
		return null;
	}
	...
	public Integer visit(ArrayAllocationExpression n,tab s) {
		...
		reg=stack.pop();
		int sizeReg = (++reg);
		if(sizeReg!=tempReg) Println(("mov temp " + (sizeReg)+" temp "+tempReg),s);
		
		/*prevent f3<0*/
		/*f3>=0 is legal*/
		Println(("cjump lt temp " + (sizeReg) + " 0 noerror"+ (++noerror)),s);
		Println(("error"),s);
		Println(("noerror"+noerror+" noop"),s);
		/*if size==0 , there will be one word,just size itself*/
		Println(("mov temp " + (sizeReg)+" plus 1 temp "+sizeReg),s);
		/*sizeReg here is the real number of words stored in memory*/
		
		int resReg = (++reg);
		Println(("mov temp " + (resReg) + " hallocate times 4 temp "+ sizeReg),s);
		/*sizeReg-=1,store array-size*/
		Println(("mov temp " + (sizeReg)+" minus temp "+sizeReg+" 1"),s);
		Println(("store temp " + (resReg)+" 0 temp "+sizeReg),s);
		...
		int outlabel=(++label);
		Println(("mov temp " + (indexReg)+" 1"),s );
		Println(("mov temp " + (storeReg)+" plus temp "+resReg+" 4"),s );
		
		Println(("whilelabel"+(whilelabel)+" noop"),s );
		Println(("cjump temp "+indexReg+" temp " + (sizeReg)+" outlabel"+(outlabel)),s );
		Println(("store temp " + (storeReg)+" 0 0 "),s );
		Println(("mov temp " + (indexReg)+" plus temp "+indexReg+" 1"),s );
		Println(("mov temp " + (storeReg)+" plus temp "+storeReg+" 4"),s );
		Println(("cjump whilelabel"+(whilelabel)),s );
		Println(("outlabel"+(outlabel)+" noop"),s );
		return resReg;
	}
	...
	public Integer visit(AllocationExpression n,tab s) {
		...
		while (curr != null) {
			...
			classSize = varNum + 2;
			Println(("mov temp " + (classReg) + " hallocate "+ (4 * (classSize))),s);
			Println(("mov temp " + (funcReg) + " hallocate "+ (4 * funcNum)),s);
			// store functions
			for (i = 0; i < funcNum; i++) {
				Println(("hstore temp " + (funcReg) + " " + (4 * i)+ " " + getProcess(curr, i)),s);
			}
			Println(("hstore temp " + (classReg) + " 0 " + "temp "+ (funcReg)),s);

			// initialize vars
			for (i = 0; i < varNum; i++) {
				Println(("hstore temp " + (classReg) + " "+ (4 * (i + 1)) + " 0"),s);
			}

			// to return resReg
			if (curr == table) {
				Println(("mov temp " + resReg + " temp " + classReg),s);
			}
			// put class pointer(reg) into memory
			else {
				Println(("hstore temp " + classStoreReg + " 0 "+ "temp " + classReg),s);
			}
			// copy father-reg
			if (curr.father != null) {
				Println(("mov temp " + classStoreReg + " plus "+ "temp " + classReg + " " + (4 * (classSize - 1))),s);
				curr = curr.father;
			}
			// father is null
			else {
				Println(("hstore temp " + classReg + " "+ (4 * (classSize - 1)) + " 0 "),s);
				break;
			}
		}
		return resReg;
	...
	public Integer visit(NotExpression n,tab s) {
		...
		reg=stack.pop();
		int preReg=(++reg);
		if(preReg!=tempReg) Println(("mov temp " + (preReg)+" temp "+tempReg),s);
		int resReg=(++reg);
