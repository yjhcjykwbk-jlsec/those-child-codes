...
public class GJDepthFirst implements GJVisitor<Object, st> {
	...
	public Object visit(StmtExp n, st argu) {
		...
		PriorityQueue<colorReg> queue=new PriorityQueue(30,comp);
		Stack<colorReg> stack=new Stack<colorReg>();
		for (int i = 0; i < n1; i++) {
			for (int j = i + 1; j < n1; j++) {
			...
			queue.add(new colorReg(temps.elementAt(i),degree[i]));
		}
		/* color use 15 colors*/
		colorReg reg=queue.poll();
		while(reg!=null&&reg.degree<15)
		{
			System.out.println(reg.num+":"+reg.degree);
			stack.push(reg);
			for (int i = 0; i < n1; i++);//delete this reg
			reg=queue.poll();
		}
		n.f3.accept(this, argu);
		return null;
